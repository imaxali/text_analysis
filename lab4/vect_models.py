...
class Estimation:
    ...
    def jaccard_sim(self, md, w1, w2):
        ...
        for w1, w2 in zip(md.toarray()[self.word1_i], md.toarray()[self.word2_i]):
            max_frq += max(w1, w2)
            min_frq += min(w1, w2)
    ...
    def cosine_sim(self, md, w1, w2):
        ...
        for v1, v2 in zip(md.toarray()[self.word1_i], md.toarray()[self.word2_i]):
            dot_product += v1 * v2
            v1_magnitude += v1 * v1
            v2_magnitude += v2 * v2
    ...
    def pearson_corrcoef(self):
        m = sparse.vstack((self.tf_idf, self.tf_idf), format='csr', dtype=float)
        n = m.shape[1]
        ...
        # coeffs = np.corrcoef(self.doc_term.todense(), self.tf_idf.todense())
        return coeffs
